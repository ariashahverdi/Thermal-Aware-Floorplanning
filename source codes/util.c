 ...
 */
int parse_cmdline(str_pair *table, int max_entries)
{
	/*int i, count;
	...
	strncpy(table[0].name, "c", STR_SIZE-1);
	table[0].name[STR_SIZE-1] = '\0';
	strncpy(table[0].value, "hotspot.config", STR_SIZE-1);
	table[0].value[STR_SIZE-1] = '\0';
	
	strncpy(table[1].name, "init_file", STR_SIZE-1);
	table[1].name[STR_SIZE-1] = '\0';
	strncpy(table[1].value, "gcc.init", STR_SIZE-1);
	table[1].value[STR_SIZE-1] = '\0';
	
	strncpy(table[2].name, "f", STR_SIZE-1);
	table[2].name[STR_SIZE-1] = '\0';
	strncpy(table[2].value, "ev6.flp", STR_SIZE-1);
	table[2].value[STR_SIZE-1] = '\0';
	
	strncpy(table[3].name, "p", STR_SIZE-1);
	table[3].name[STR_SIZE-1] = '\0';
	strncpy(table[3].value, "gcc.ptrace", STR_SIZE-1);
	table[3].value[STR_SIZE-1] = '\0';
