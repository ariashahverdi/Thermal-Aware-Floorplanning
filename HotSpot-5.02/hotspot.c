...
double get_temp(int argc, char **argv)
{
    ...
    */
    ...
    /* Aria: Compute The maximum temperature */
    double max_temp;
    QuickSort(steady_temp,0,n-1);
    max_temp = steady_temp[n-1];
	...
	if (model->type == BLOCK_MODEL) {
		if (do_transient) {
			fprintf(stdout, "1. printing temp...\n");
			dump_dvector(temp, model->block->n_nodes);
		}
		fprintf(stdout, "2. printing steady_temp...\n");
		dump_dvector(steady_temp, model->block->n_nodes);
	} else {
		if (do_transient) {
			fprintf(stdout, "3. printing temp...\n");
			dump_dvector(temp, model->grid->total_n_blocks + EXTRA);
		}
		fprintf(stdout, "4. printing steady_temp...\n");
    ...
    int main(void)
    {
        ...
        char outputFilename[] = "113077548.txt";
        ...
        double cur_temp = get_temp(5, argv);
        ...
        //Write Data
        fprintf(ofp,"The result of floorplanning is as follows...\n");
